...
local RUNTIME_KEY = "__FourHub.vs_CODE_SNIPER_RUNTIME"
...
local function createHeadDisplay()
    ...
    local old = head:FindFirstChild("FourHub.vsCodeSniperHeadDisplay")
    if old then old:Destroy() end

    local billboard = new("BillboardGui", {
        Name = "FourHub.vsCodeSniperHeadDisplay",
        Adornee = head,
...
local function isOurGui(instance)
    ...
    for _ = 1, 10 do
        ...
        if p.Name == "FourHub.vsCodeSniperUI" or p.Name == "FourHub.vsSettingsUI" then return true end
        p = p.Parent
    end
    return false
end
...
local oldGui = parentGui:FindFirstChild("FourHub.vsCodeSniperUI")
if oldGui then oldGui:Destroy() end

local ScreenGui = new("ScreenGui", {
    Name = "FourHub.vsCodeSniperUI",
    ResetOnSpawn = false,
    IgnoreGuiInset = true,
    DisplayOrder = 999,
}, parentGui)
...
local function createKeybindsWindow()
    local old = parentGui:FindFirstChild("FourHub.vsSettingsUI")
    if old then old:Destroy() end

    local SG = new("ScreenGui", {
        Name = "FourHub.vsSettingsUI",
        ResetOnSpawn = false,
        IgnoreGuiInset = true,
        DisplayOrder = 998,
    }, parentGui)
